...
public class IapController : MonoBehaviour, IStoreListener
{
    ...
    public void Init()
    {
        ...
        for (int i = 0; i < inappDatabase.lstPacksInapp.Count; i++)
        {
            ...
            // IDs cho ProductID chính
            var ids = new IDs();
            ids.Add(pack.ProductID, AppleAppStore.Name);
            ids.Add(pack.ProductID, GooglePlay.Name);

            builder.AddProduct(pack.ProductID, pack.productType );

            if (pack.isSale)
            {
                // IDs cho ProductID gốc (Origin)
                var idsOrigin = new IDs();
                idsOrigin.Add(pack.ProductID_Origin, AppleAppStore.Name);
                idsOrigin.Add(pack.ProductID_Origin, GooglePlay.Name);

                builder.AddProduct(pack.ProductID_Origin, pack.productType );
            }
        }
    ...
    public void RestorePurchases()
    {
            ...
            Application.platform == RuntimePlatform.OSXPlayer)
        {
            ...
            {
                Debug.Log($"RestorePurchases result: success={success}, error={error}");
                ...
                {
                    // error có thể null khi thành công, hoặc chứa thông báo khi lỗi
